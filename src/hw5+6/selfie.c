...
uint64_t CHAR_LEFTBRACKET  = '[';
uint64_t CHAR_RIGHTBRACKET = ']';
uint64_t CHAR_OR           = '|';
uint64_t CHAR_AND          = '&';
...
uint64_t power_of_two_table[64];
...
void initLibrary() {
  ...
  power_of_two_table[0] = 1;


  for (i = 1; i < CPUBITWIDTH; i = i + 1) {
...
uint64_t SYM_SLL          = 28; // <<
uint64_t SYM_SRL          = 29; // >>
uint64_t SYM_LEFTBRACKET  = 30; // [
uint64_t SYM_RIGHTBRACKET = 31; // ]
uint64_t SYM_STRUCT       = 32; // struct
uint64_t SYM_RIGHTARROW   = 33; // ->
uint64_t SYM_FOR          = 34; // for
uint64_t SYM_ORL          = 35; // ||
uint64_t SYM_ANDL         = 36; // &&
uint64_t SYM_NOTL         = 37; // !
...
void initScanner () {
  SYMBOLS = smalloc((SYM_NOTL + 1) * SIZEOFUINT64STAR);
  ...
  *(SYMBOLS + SYM_LEFTBRACKET)  = (uint64_t) "[";
  *(SYMBOLS + SYM_RIGHTBRACKET) = (uint64_t) "]";
  *(SYMBOLS + SYM_STRUCT)       = (uint64_t) "struct";
  *(SYMBOLS + SYM_RIGHTARROW)   = (uint64_t) "->";
  *(SYMBOLS + SYM_FOR)          = (uint64_t) "for";
  *(SYMBOLS + SYM_ANDL)         = (uint64_t) "&&";
  *(SYMBOLS + SYM_ORL)          = (uint64_t) "||";
  *(SYMBOLS + SYM_NOTL)         = (uint64_t) "!";
...
uint64_t* getNextLL(uint64_t* element)                { return (uint64_t*) *element; }
void setNextLL(uint64_t* element, uint64_t* next)     { *element = (uint64_t) next; }
uint64_t* allocateElementLL(uint64_t* in);

// LinkedList Struct Array
// +----+----------------+
// |  0 | nextElement    | pointer to next context
// |  1 | Payload        | payload
// +----+----------------+

void printArrayLL(uint64_t* ll);
uint64_t* revertArrayLL(uint64_t* ll);

uint64_t getArraySize(uint64_t* arrayDimList);
void computeArrayOffset(uint64_t* arrayDimList);

uint64_t getPayloadLL(uint64_t* element)                 { return *(element + 1); }
void setPayloadLL(uint64_t* element, uint64_t payload)   { *(element + 1)  =  payload; }

// LinkedList Struct Struct
// +----+----------------+
// |  0 | nextElement     | pointer to next context
// |  1 | fieldname       |
// |  2 | offset          | offset from base address
// |  3 | type            |
// |  4 | structTypeName  | only if type = STRUCT_T
// +----+----------------+

void printStructLL(uint64_t* fieldList);
uint64_t* findStructField(uint64_t* fieldList, uint64_t* fieldName);

uint64_t* getFieldName(uint64_t* element)            { return (uint64_t*) *(element + 1); }
uint64_t  getFieldOffset(uint64_t* element)          { return *(element + 2); }
uint64_t  getFieldType(uint64_t* element)            { return *(element + 3); }
uint64_t* getFieldStructTypeName(uint64_t* element)  { return (uint64_t*) *(element + 4); }

void setFieldName(uint64_t* element, uint64_t* name)             { *(element + 1)  =  (uint64_t) name; }
void setFieldOffset(uint64_t* element, uint64_t offset)          { *(element + 2)  =  offset; }
void setFieldType(uint64_t* element, uint64_t type)              { *(element + 3)  =  type; }
void setFieldStructTypeName(uint64_t* element, uint64_t* name)   { *(element + 4)  =  (uint64_t) name; }
...
// |  8 | arrayDimList  | Linked List of max dimensions or null
// |  9 | structFieldList  | Linked List of struct fieldnames, types and offsets or null
// +----+---------+
...
struct symbol_table_t {
  ...
  uint64_t address;
  uint64_t scope;
  uint64_t* arrayDimList;
  uint64_t* structFieldList;
};

struct symbol_table_t * newEntry;

uint64_t* getNextEntry(struct symbol_table_t* entry)        { return (uint64_t*) entry->next; }
uint64_t* getString(struct symbol_table_t* entry)           { return (uint64_t*) entry->string; }
...
uint64_t  getType(struct symbol_table_t* entry)             { return             entry->type; }
...
uint64_t* getArrayDimList(struct symbol_table_t* entry)     { return (uint64_t*) entry->arrayDimList; }
uint64_t* getStructFieldList(struct symbol_table_t* entry)  { return (uint64_t*) entry->structFieldList; }

void setNextEntry(struct symbol_table_t* entry, uint64_t* next)                      { entry->next       = (uint64_t) next; }
void setString(struct symbol_table_t* entry, uint64_t* identifier)                   { entry->string = (uint64_t) identifier; }
...
void setType(struct symbol_table_t* entry, uint64_t type)                            { entry->type = type; }
...
void setArrayDimList(struct symbol_table_t* entry, uint64_t* arrayDimList)           { entry->arrayDimList = (uint64_t) arrayDimList; }
void setStructFieldList(struct symbol_table_t* entry, uint64_t* structFieldList)     { entry->structFieldList = (uint64_t) structFieldList; }
// ------------------------ GLOBAL CONSTANTS -----------------------
...
// table pointers
struct symbol_table_t *  global_symbol_table_t;
struct symbol_table_t *  local_symbol_table_t;
struct symbol_table_t *  library_symbol_table_t;
...
void resetSymbolTables() {
  global_symbol_table_t  = (uint64_t*) 0;
  local_symbol_table_t   = (uint64_t*) 0;
  library_symbol_table_t = (uint64_t*) 0;
...
uint64_t  compile_complexExpression();
...
void compile_structDefinition(uint64_t* structName);
void compile_structDeclaration(uint64_t* structName);
void compile_structAccess(uint64_t* variableName);
...
void emitSRL(uint64_t rd, uint64_t rs1, uint64_t rs2);
...
uint64_t maxBinaryLength = 524288; // 256KB
...
uint64_t identifierOrKeyword() {
  if (identifierStringMatch(SYM_WHILE))
    return SYM_WHILE;
  if (identifierStringMatch(SYM_FOR))
    return SYM_FOR;
...
void getSymbol() {
  ...
  if (findNextCharacter() != CHAR_EOF) {
    if (symbol != SYM_DIV) {
      ...
      } else if (character == CHAR_LEFTBRACKET) {
        getCharacter();

        symbol = SYM_LEFTBRACKET;

      } else if (character == CHAR_RIGHTBRACKET) {
        getCharacter();
...
void createSymbolTableEntry(uint64_t whichTable, uint64_t* string, uint64_t line, uint64_t class, uint64_t type, uint64_t value, uint64_t address) {
  ...
  newEntry = smalloc(4 * SIZEOFUINT64STAR + 6 * SIZEOFUINT64);
  ...
  newEntry->arrayDimList = (uint64_t*) 0;
  newEntry->structFieldList = (uint64_t*) 0;

  // create entry at head of symbol table
  if (whichTable == GLOBAL_TABLE) {
    setScope(newEntry, REG_GP);
    setNextEntry(newEntry, global_symbol_table_t);
    global_symbol_table_t = newEntry;
    ...
    setNextEntry(newEntry, local_symbol_table_t);
    local_symbol_table_t = newEntry;
  } else {
    // library procedures
    setScope(newEntry, REG_GP);
    setNextEntry(newEntry, library_symbol_table_t);
    library_symbol_table_t = newEntry;
  }
}
...
uint64_t* getScopedSymbolTableEntry(uint64_t* string, uint64_t class) {
  ...
  if (class == VARIABLE)
    ...
    entry = searchSymbolTable(local_symbol_table_t, string, VARIABLE);
  else if (class == PROCEDURE)
    // library procedures override declared or defined procedures
    entry = searchSymbolTable(library_symbol_table_t, string, PROCEDURE);
  else
    entry = (uint64_t*) 0;

  if (entry == (uint64_t*) 0)
    return searchSymbolTable(global_symbol_table_t, string, class);
  else
    return entry;
}
...
uint64_t isUndefinedProcedure(uint64_t* entry) {
  ...
  if (getClass(entry) == PROCEDURE) {
    // library procedures override declared or defined procedures
    libraryEntry = searchSymbolTable(library_symbol_table_t, getString(entry), PROCEDURE);
...
uint64_t* getVariableOrBigInt(uint64_t* variableOrBigInt, uint64_t class) {
  ...
  if (class == BIGINT)
    return searchSymbolTable(global_symbol_table_t, variableOrBigInt, class);
  else {
...
void load_integer(uint64_t value) {
  ...
  if (isSignedInteger(value, 12)) {
    ...
    entry = searchSymbolTable(global_symbol_table_t, integer, BIGINT);
...
uint64_t compile_factor() {
  ...
  // identifier?
    ...
    if (symbol == SYM_LPARENTHESIS) {
    ...
    } else if (symbol == SYM_LEFTBRACKET) {
      entry = getVariableOrBigInt(variableOrProcedureName, VARIABLE);
      computeArrayOffset(getArrayDimList(entry));
      ...
      emitLD(currentTemporary(), currentTemporary(), 0);
    } else if(symbol == SYM_RIGHTARROW) {
			getSymbol();

			compile_structAccess(variableOrProcedureName);
  ...
  //  "(" expression ")"
    ...
    print((uint64_t*) "aaa");println();
    // not !
    emitBEQ(currentTemporary(), REG_ZR, 12);
    emitADDI(currentTemporary(), REG_ZR, 0);
    emitBEQ(REG_ZR, REG_ZR, 8);
...
uint64_t compile_logicalOr() {
  ...
  uint64_t rtype;
	...
	uint64_t* fixUpChainLast;
	uint64_t* fixUpChainNew;
	brForwardToEnd = 0;
	fixUpChainLast = (uint64_t*) 0;
	fixUpChainNew = (uint64_t*) 0;

	ltype = compile_logicalAnd();

	while(symbol == SYM_ORL) {
		getSymbol();

		fixUpChainNew = malloc(8);
		*(fixUpChainNew + 0) = (uint64_t) fixUpChainLast;
		fixUpChainLast = fixUpChainNew;

    emitSLTU(currentTemporary(), REG_ZR, currentTemporary());

    load_integer(1);
    brForwardToEnd = binaryLength;
    *(fixUpChainNew + 1) = brForwardToEnd;
    emitBEQ(previousTemporary(), currentTemporary(), 0);
	...
	while(fixUpChainNew != (uint64_t*) 0) {
		fixup_relative_BFormat(*(fixUpChainNew + 1));
		fixUpChainNew = (uint64_t*) *(fixUpChainNew + 0);
	}
...
uint64_t compile_logicalAnd() {
  ...
  uint64_t rtype;
	...
	while(fixUpChainNew != (uint64_t*) 0) {
		//printInteger(*(fixUpChainNew + 1));print((int*) "-");
		//printInteger(*(fixUpChainNew + 0));println();
		fixup_relative_BFormat(*(fixUpChainNew + 1));
		fixUpChainNew = (uint64_t*) *(fixUpChainNew + 0);
	}
...
uint64_t compile_expression() {
  ...
  ltype = compile_complexExpression();
  ...
  if (isComparison()) {
    ...
    rtype = compile_complexExpression();
...
uint64_t compile_complexExpression() {
  ...
  while (isLeftOrRightShift()) {
    ...
    } else if (operatorSymbol == SYM_SRL) {
      if (rtype != UINT64_T)
        typeWarning(ltype, rtype);
...
void compile_for() {
  uint64_t jumpBackToIncrement;
  uint64_t jumpBackToWhile;
  uint64_t branchForwardToEnd;
  uint64_t branchForwardToBody;

  // assert: allocatedTemporaries == 0


  branchForwardToEnd = 0;
  branchForwardToBody = 0;
  ...
  if (symbol == SYM_FOR) {
    ...
    if (symbol == SYM_LPARENTHESIS) {
      ...
      branchForwardToBody = binaryLength;
      emitBEQ(REG_ZR, REG_ZR, 0);

      jumpBackToIncrement = binaryLength;
      getSymbol(); // ;
      compile_statement();
      ...
      if (symbol == SYM_RPARENTHESIS) {
        ...
        if (symbol == SYM_LBRACE) {
          ...
          if (branchForwardToBody != 0)
            fixup_relative_BFormat(branchForwardToBody);
  ...
  emitJAL(REG_ZR, jumpBackToIncrement - binaryLength);
...
uint64_t compile_variable(uint64_t localVariables) {
  uint64_t type;
  uint64_t* arrayDimList;
  uint64_t* entry;
  uint64_t* variableName;
  uint64_t* structTypeName;
  ...
  if (type == STRUCT_T) {
    ...
    createSymbolTableEntry(LOCAL_TABLE, variableName, lineNumber, VARIABLE, type, 0, -localVariables * REGISTERSIZE);
    entry = getVariableOrBigInt(variableName, VARIABLE);
    setStructFieldList(entry, getStructFieldList(getVariableOrBigInt(structTypeName, TYPE)));
    ...
    if(symbol == SYM_LEFTBRACKET) {
      arrayDimList = compile_selector();

      localVariables = localVariables + getArraySize(arrayDimList);

      createSymbolTableEntry(LOCAL_TABLE, variableName, lineNumber, VARIABLE, type, 0, -localVariables * REGISTERSIZE);
      entry = getVariableOrBigInt(variableName, VARIABLE);
      setArrayDimList(entry, arrayDimList);
    ...
    createSymbolTableEntry(LOCAL_TABLE, (uint64_t*) "missing variable name", lineNumber, VARIABLE, type, 0, -localVariables * REGISTERSIZE);
  }
...
void compile_procedure(uint64_t* procedure, uint64_t type) {
  ...
  entry = searchSymbolTable(global_symbol_table_t, procedure, PROCEDURE);
  ...
  if (symbol == SYM_SEMICOLON) {
    ...
    while (symbol == SYM_UINT64) {
      ...
      localVariables = compile_variable(localVariables);
  ...
  while (symbol != SYM_EOF) {
    ...
    if (symbol == SYM_VOID) {
      ...
      if (symbol == SYM_IDENTIFIER) {
			...
			if (symbol == SYM_LBRACE) {
				getSymbol();
				compile_structDefinition(structTypeName);
			} else if (symbol == SYM_ASTERISK) {
				getSymbol();
				compile_structDeclaration(structTypeName);
      ...
      if (symbol == SYM_IDENTIFIER) {
        ...
        if (symbol == SYM_LPARENTHESIS)
          ...
          entry = searchSymbolTable(global_symbol_table_t, variableOrProcedureName, VARIABLE);

          if (entry == (uint64_t*) 0) {
             if (arrayDimList != (uint64_t*) 0) {
               allocatedMemory = allocatedMemory + REGISTERSIZE * getArraySize(arrayDimList);
...
void compile_structDefinition(uint64_t* structName) {
  ...
  offset = 0;
  fieldList = (uint64_t*) 0;


  while(lookForType() == 0) {
    fieldStructTypeName = (uint64_t*) 0;
    fieldName = (uint64_t*) 0;
    ...
    if(symbol == SYM_STRUCT) {
      ...
      type = STRUCTSTAR_T;
			...
			if(symbol == SYM_IDENTIFIER) {
				fieldStructTypeName = identifier;
				getSymbol();
      }
    ...
    //create fielNameList entry and append
    fieldList = allocateElementLL(fieldList);
    setFieldName(fieldList,fieldName);
    setFieldOffset(fieldList,offset);
    setFieldType(fieldList,type);
    setFieldStructTypeName(fieldList,fieldStructTypeName);

    offset = offset + REGISTERSIZE;
    getSymbol();
  ...
  //printStructLL(fieldList);

  createSymbolTableEntry(GLOBAL_TABLE, structName, lineNumber, TYPE, STRUCT_T, 0, 0);
  entry = searchSymbolTable(global_symbol_table_t, structName, TYPE);
  setStructFieldList(entry, fieldList);
  ...
  //println();print((uint64_t*) "After struct type definition");println();
}
...
void compile_structDeclaration(uint64_t* structName) {
  ...
  entry = searchSymbolTable(global_symbol_table_t, structName, TYPE);
	...
	if(entry != (uint64_t*) 0) {
		...
		createSymbolTableEntry(GLOBAL_TABLE, variableName, lineNumber, VARIABLE, STRUCTSTAR_T, 0, -allocatedMemory);

		fieldList = getStructFieldList(entry);
		entry = searchSymbolTable(global_symbol_table_t, variableName, VARIABLE);
		setStructFieldList(entry, fieldList);

		//printStructLL(fieldList);
	} else {
		printLineNumber((uint64_t*) "warning", lineNumber);
		print((uint64_t*) "struct of type ");
		print(structName);
		print((uint64_t*) " not defined");
		println();
	}
...
void compile_structAccess(uint64_t* variableName) {
  ...
  uint64_t* fieldList;
  uint64_t* typeEntry;
  uint64_t* variableEntry;
  uint64_t* tempField;
  uint64_t* field;

  variableEntry = getVariableOrBigInt(variableName, VARIABLE);
  fieldList = getStructFieldList(variableEntry);
  load_variableOrBigInt(variableName, VARIABLE);
  ...
  while(symbol == SYM_IDENTIFIER) {
    ...
    field = findStructField(fieldList, fieldName);

    if(field != (uint64_t*) 0) {

      //printStructLL(field);
      if(symbol == SYM_RIGHTARROW) {
        typeEntry = getVariableOrBigInt(getFieldStructTypeName(field), TYPE);

        emitLD(currentTemporary(),currentTemporary(),getFieldOffset(field));
        fieldList = getStructFieldList(typeEntry);

        getSymbol();
      } else {
        emitADDI(currentTemporary(), currentTemporary(),getFieldOffset(field));
      }
    }
  }
}
...
uint64_t* compile_selector() {
  uint64_t payload;
  uint64_t* dimensionList;

  dimensionList = (uint64_t*) 0;

  while(symbol == SYM_LEFTBRACKET) {
    getSymbol();
    compile_simpleExpression();

    dimensionList = allocateElementLL(dimensionList);
    setPayloadLL(dimensionList, literal);

    getSymbol();
    tfree(1);
  }

  dimensionList = revertArrayLL(dimensionList);
  //printArrayLL(dimensionList);
...
void printRegister(uint64_t reg) {
  print((uint64_t*) REGISTERS[ABBR][reg]);
}
...
void emitSRL(uint64_t rd, uint64_t rs1, uint64_t rs2) {
  emitInstruction(encodeRFormat(F7_SRL, rs2, rs1, F3_SRL, rd, OP_OP));
...
void emitGlobalsStrings() {
  ...
  while ((uint64_t) entry != 0) {
    if (getClass(entry) == VARIABLE) {
      ...
      if(getArrayDimList(entry) == (uint64_t*) 0)
        binaryLength = binaryLength + REGISTERSIZE;
      else
        binaryLength = binaryLength + REGISTERSIZE * getArraySize(getArrayDimList(entry));
      ...
      if (funct7 == F7_DIVU) {
        ...
        if (debug) {
          if (record) {
            ...
            print_add_sub_mul_divu_remu_sltu((uint64_t*) "srl");
            if (execute) {
              print_add_sub_mul_divu_remu_sltu_before();
              do_srl();
              print_addi_add_sub_mul_divu_remu_sltu_after();
      ...
      if (funct7 == F7_SLL) {
        if (debug) {
          if (record) {
            ...
            print_add_sub_mul_divu_remu_sltu((uint64_t*) "sll");
            if (execute) {
              print_add_sub_mul_divu_remu_sltu_before();
              do_sll();
              print_addi_add_sub_mul_divu_remu_sltu_after();
...
// -----------------------------------------------------------------
uint64_t getArraySize(uint64_t* arrayDimList) {
  uint64_t* tempElement;
  uint64_t size;

  size = 1;
  tempElement = arrayDimList;

  while (tempElement != (uint64_t*) 0) {
    size = size * getPayloadLL(tempElement);
    tempElement = getNextLL(tempElement);
  }

  //size is number or machinewords
  return size;
}
...
void computeArrayOffset(uint64_t* arrayDimList) {
  ...
  while(symbol == SYM_LEFTBRACKET) {
    getSymbol();
    compile_simpleExpression();
    load_integer(getArraySize(getNextLL(arrayDimList)));

    //print((uint64_t*) "Dimension Weight: ");
    //printInteger(getArraySize(getNextLL(arrayDimList)));println();
    ...
    arrayDimList = getNextLL(arrayDimList);
    getSymbol();
    tfree(1);
  }
...
void printArrayLL(uint64_t* ll) {
  ...
  tempElement = ll;
  println(); print((uint64_t*) "Linkedlist" ); println();
  print((uint64_t*) "# | nextpointer | payload" ); println();

  i = 1;
  while (tempElement != (uint64_t*) 0) {
    print((uint64_t*) "#" ); printInteger(i);
    print((uint64_t*) "  " ); printInteger((uint64_t) getNextLL(tempElement));
    print((uint64_t*) "  " ); printInteger(getPayloadLL(tempElement));
...
void printStructLL(uint64_t* fieldList) {
  ...
  tempElement = fieldList;
  println(); print((uint64_t*) "Linkedlist" ); println();
  print((uint64_t*) "# | nextpointer | offset | type | name | structtype" ); println();

  i = 1;
  while (tempElement != (uint64_t*) 0) {
    print((uint64_t*) "#" ); printInteger(i);
    print((uint64_t*) "  " ); print(getFieldName(tempElement));
    print((uint64_t*) "  " ); printInteger(getFieldType(tempElement));
    print((uint64_t*) "  " ); printInteger(getFieldOffset(tempElement));
    print((uint64_t*) "  " ); printInteger((uint64_t) getNextLL(tempElement));
    print((uint64_t*) "  " ); print(getFieldStructTypeName(tempElement));
...
uint64_t* findStructField(uint64_t* fieldList, uint64_t* fieldName) {
  ...
  while(fieldList != (uint64_t*) 0) {
    if (stringCompare(getFieldName(fieldList), fieldName)) {
      return fieldList;
    }
    fieldList = getNextLL(fieldList);
  }
...
uint64_t* allocateElementLL(uint64_t* in) {
  uint64_t* element;
  element = malloc(SIZEOFUINT64STAR * 3 + SIZEOFUINT64 * 2);

  setNextLL(element, in);
...
uint64_t* revertArrayLL(uint64_t* ll) {
  ...
  while (tempElement != (uint64_t*) 0) {
    tempLL = allocateElementLL(tempLL);
    setPayloadLL(tempLL,getPayloadLL(tempElement));
    tempElement = getNextLL(tempElement);
  }
...
uint64_t selfie() {
  ...
  println();
  print((uint64_t*) "This is Sebastian Hattinger's Selfie");
  println();
  //test();
  println();
