...
void initScanner () {
  SYMBOLS = smalloc((SYM_RIGHTBRACKET + 1) * SIZEOFUINT64STAR);
...
// |  8 | arrayDimList  | Linked List of max dimensions or nullpointer
// +----+---------+
...
uint64_t* getArrayDimList(uint64_t* entry)  { return (uint64_t*) *(entry + 8); }
...
void setArrayDimList(uint64_t* entry, uint64_t* arrayDimList)  { *(entry + 8) = (uint64_t) arrayDimList; }
// ------------------------ GLOBAL CONSTANTS -----------------------
...
// |  1 | Payload        | context to execute
// +----+----------------+

void printLL(uint64_t* ll);
uint64_t getArraySize(uint64_t* arrayDimList);
void computeArrayOffset(uint64_t* arrayDimList);
uint64_t* allocateElementLL(uint64_t* in, uint64_t payload);
uint64_t* revertLL(uint64_t* ll);
...
void createSymbolTableEntry(uint64_t whichTable, uint64_t* string, uint64_t line, uint64_t class, uint64_t type, uint64_t value, uint64_t address) {
  ...
  newEntry = smalloc(3 * SIZEOFUINT64STAR + 6 * SIZEOFUINT64);
  ...
  setArrayDimList(newEntry, (uint64_t*) 0);
        ...
        if (symbol == SYM_LPARENTHESIS)
          ...
          if (entry == (uint64_t*) 0) {
             if (arrayDimList != (uint64_t*) 0) {
               //print((uint64_t*) "ArraySize:"); printInteger(getArraySize(arrayDimList));println();
               //print((uint64_t*) "AllocatedMemoryBefore:"); printInteger(allocatedMemory);println();
               allocatedMemory = allocatedMemory + REGISTERSIZE * getArraySize(arrayDimList);
               //print((uint64_t*) "AllocatedMemoryAfter:"); printInteger(allocatedMemory);println();
...
uint64_t* compile_selector() {
  ...
  while(symbol == SYM_LEFTBRACKET) {
    ...
    dimensionList = allocateElementLL(dimensionList, literal);

    getSymbol();
    tfree(1);
  }

  dimensionList = revertLL(dimensionList);
  printLL(dimensionList);
...
uint64_t* allocateElementLL(uint64_t* in, uint64_t payload) {
  uint64_t* element;
  element = malloc(SIZEOFUINT64STAR + SIZEOFUINT64);

  setNextLL(element, in);
  setPayloadLL(element, payload);
...
uint64_t* revertLL(uint64_t* ll) {
  ...
  while (tempElement != (uint64_t*) 0) {
    tempLL = allocateElementLL(tempLL, getPayloadLL(tempElement));
    tempElement = getNextLL(tempElement);
  }
...
uint64_t test() {
  ...
  ll = (uint64_t*) 0;
  print((uint64_t*) "TEST");
  ll = allocateElementLL(ll, 1);
  ll = allocateElementLL(ll, 2);
  ll = allocateElementLL(ll, 3);
  ll = allocateElementLL(ll, 4);
  //printLL(ll);
  ll = revertLL(ll);
  //printLL(ll);
}
